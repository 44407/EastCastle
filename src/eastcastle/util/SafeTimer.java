...
/**
 * Extends Timer to ensure that all called TimerTasks are wrapped in
 * SafeTimerTasks. This prevents the Timer from breaking due to a thrown
 * exception in a TimerTask.
 */
public class SafeTimer extends Timer {
    ...
    *                                  or {@code period <= 0} ,or the timer task is
    *                                  not a SafeTimerTask
    * @throws IllegalStateException    if task was already scheduled or cancelled,
    *                                  timer was cancelled, or timer thread
    *                                  terminated.
